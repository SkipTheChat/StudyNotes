...
项目架构是springboot+springcloud，主要分为一级域名，和二级域名，一级域名面向用户，实现的功能有：

1.使用JWT和非对称加密完成用户的登录，使用消息中间件和redis完成用户的注册

2.使用elasticsearch实现用户对商品的实时搜索

3.点击商品进入详情页，这里使用thymleaf对页面进行缓存

4.并将商品加入到购物车，商品信息由redis进行保存
...
4.所有的商品图片使用FastDFS进行存储



> 主要用到的技术有：

**elasticsearch：**主要负责主页用户实时搜索商品

**FastDFS：**用于商家上架商品时图片的上传

**rabbitMQ：**是内部的消息中间件，负责两个模块：

1.在注册时发送验证码，rabbitMQ调用sms模块发送验证给用户；

2.在商家更新商品信息的时候，需要更新elasticsearch的存储信息，防止后台数据库和elasticsearch存储的信息不一致，所以需要用rabbitMQ去通知elasticsearch进行更新
...
![TIM截图20200304123539](./assets/TIM%E6%88%AA%E5%9B%BE20200304123539.png)
